...
//start button
var startBtn = $("<button>");
startBtn.attr("id", "start-button-hermoine");
startBtn.text("Start");
...
potionsPic.css("max-width", "639px")
...
$("#hermione-game-nav").on("click", function (event) {
    ...
    $(".myText").textillate({ in: { effect: 'fadeIn', shuffle: 'true', delay: 10 }});

});

$(document).on("click", "#start-button-hermoine", function () {
    $("#main-content-area").empty();
    $("#main-content-area").append(containerTwo);
    $(".myText").textillate({ in: { effect: 'fadeIn', shuffle: 'true', delay: 5 }});
    // answerQuestion();
